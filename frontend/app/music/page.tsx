      ...
      <main style={{ maxWidth: "1400px", margin: "0 auto", padding: "24px 24px 100px 24px" }}>
        ...
        <div style={{ padding: "24px", borderRadius: "20px", background: "var(--palette-gradient-music)", marginBottom: "32px", boxShadow: `0 4px 20px var(--palette-shadow-xl)` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            <div style={{ width: "60px", height: "60px", borderRadius: "12px", backgroundColor: "var(--palette-surface)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px" }}>❄️</div>
            <div style={{ flex: 1 }}>
              <h3 style={{ fontSize: "18px", fontWeight: "700" }}>Snowfall</h3>
              <p style={{ fontSize: "14px", color: "var(--palette-text-secondary)" }}>Øneheart · 3:24</p>
            </div>
          ...
          {/* Progress Bar */}
          <div style={{ marginTop: "16px", height: "4px", borderRadius: "999px", backgroundColor: "var(--palette-progress-track)" }}>
            <div style={{ width: "35%", height: "100%", borderRadius: "999px", backgroundColor: "var(--palette-progress-fill)" }} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "8px", marginBottom: "24px" }}>
          {moods.map(mood => (
            <button
              key={mood}
              style={{ padding: "8px 16px", borderRadius: "999px", fontSize: "13px", fontWeight: "500", whiteSpace: "nowrap", cursor: "pointer", backgroundColor: mood === "All" ? "var(--palette-primary)" : "var(--palette-surface)", border: `1px solid ${mood === "All" ? "var(--palette-border-active)" : "var(--palette-border-subtle)"}`, color: mood === "All" ? "var(--palette-text)" : "var(--palette-text-secondary)" }}
            >
              {mood}
            </button>
          ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {playlists.map((song) => (
            <div
              key={song.id}
              style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px", borderRadius: "16px", backgroundColor: "var(--palette-surface)", border: "1px solid var(--palette-border)", cursor: "pointer" }}
            >
